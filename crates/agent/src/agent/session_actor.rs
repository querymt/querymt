...
impl Message<Prompt> for SessionActor {
    ...
    async fn handle(&mut self, msg: Prompt, ctx: &mut Context<Self, Self::Reply>) -> Self::Reply {
        if self.prompt_running {
            // If a cancel has been requested, allow the new prompt through.
            // The old (cancelled) task is winding down and still holds the
            // execution_permit semaphore.  The new task will wait on that
            // semaphore until the old one finishes its cleanup — no two
            // prompts will execute concurrently.
            if *self.cancel_tx.borrow() {
                ...
                debug!(
                    "Session {}: prompt_running=true, cancel=false → rejecting with 'session busy'",
                    self.session_id
                );
                return ctx.reply(Err(Error::new(-32001, "session busy")));
            }
        } else {
            debug!(
...
async fn execute_prompt_detached(
    ...
    // Store User Messages
    let full_content = format_prompt_blocks(&req.prompt, config.max_prompt_bytes);
...
mod tests {
    ...
    /// A non-cancelled busy session must still reject concurrent prompts.
    #[test]
    fn busy_session_without_cancel_rejects_new_prompt() {
        let (cancel_tx, _rx) = watch::channel(false);
        let prompt_running = true;

        let should_reject = prompt_running && !*cancel_tx.borrow();
        assert!(
            should_reject,
            "A busy, non-cancelled session must reject new prompts"
        );
    }
}
