...
impl AgentBuilderExt for QueryMTAgent {
    ...
    fn with_compaction_config(mut self, config: CompactionConfig) -> Self {
        // Store compaction config so middleware factories can read it later
        // Auto-add of ContextMiddleware is deferred until after config-based middleware is loaded
