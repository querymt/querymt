...
impl MerkleTree {
    pub fn scan(root: &Path) -> Self {
        ...
        let mut overall_hasher = Hasher::new();
        ...
        for path in paths {
            if let Ok(content) = std::fs::read(&path) {
                ...
                // Update global hash with path + content hash
                overall_hasher.update(path.to_string_lossy().as_bytes());
                overall_hasher.update(hash.as_bytes());
                entries.insert(path, hash);
            }
        }

        Self {
            root_hash: overall_hasher.finalize().to_hex().to_string(),
            entries,
        }
    }
