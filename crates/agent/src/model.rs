...
fn native_replay_allowed(
    ...
    let Some(provenance) = output.provenance.as_ref() else {
        // Without provenance the output predates provenance tracking; the
        // caller's provider/model comparison is the only available signal.
    ...
    pub fn to_chat_message_with_output_target(
        ...
        for part in &self.parts {
            match part {
                ...
                MessagePart::Output { output } => {
                    if native_replay_allowed(output, target) {
                        output_items.extend(output.items.clone());
                    } else {
                        for item in &output.items {
                            match item {
                                ChatOutputItem::Message(message) => {
                                    ...
                                    output_items.push(ChatOutputItem::Message(message));
                                }
                                ChatOutputItem::Reasoning(reasoning) => {
                                    ...
                                    output_items.push(ChatOutputItem::Reasoning(reasoning));
                                }
                                ChatOutputItem::FunctionCall(call) => {
                                    ...
                                    output_items.push(ChatOutputItem::FunctionCall(call));
                                }
        ...
        if self.role == ChatRole::Assistant {
            return Ok(ChatMessage::from_assistant_output(ChatOutput {
                items: output_items,
