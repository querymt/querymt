//! Tool: `route_delegation_to_peer` — route a delegate's session to a named peer.
//!
//! Sends `SetSessionTarget` to the `RoutingActor`, returning confirmation with
//! the current routing state so the planner gets feedback.

use crate::agent::remote::routing::{RouteTarget, RoutingActor, SetSessionTarget};
...
pub struct RouteDelegationToPeerTool {
    actor: ActorRef<RoutingActor>,
}

impl RouteDelegationToPeerTool {
...
#[async_trait]
impl ToolTrait for RouteDelegationToPeerTool {
    fn name(&self) -> &str {
        "route_delegation_to_peer"
    }
    ...
    fn definition(&self) -> Tool {
        Tool {
            ...
            function: FunctionTool {
                ...
                description: "Route a delegate agent's session to run on a named mesh peer, \
                              or reset to local. When set to a peer, the entire delegation \
                              session (including tool execution) runs on that remote node."
                    .to_string(),
                parameters: json!({
                    "type": "object",
                    "properties": {
                        ...
                        "peer_name": {
                            "type": ["string", "null"],
                            "description": "Peer hostname to route to, or null to reset to local."
                        }
                    },
                    "required": ["agent_id"]
                }),
    ...
    async fn call(&self, args: Value, _context: &dyn ToolContext) -> Result<String, ToolError> {
        ...
        let status = match &target {
            RouteTarget::Local => format!("Session for '{}' now routes locally.", agent_id),
            RouteTarget::Peer(name) => {
                format!("Session for '{}' now routes to peer '{}'.", agent_id, name)
            }
        };
...
mod tests {
    ...
    #[tokio::test]
    async fn test_route_delegation_to_peer_sets_session_target() {
        let snapshot = new_routing_snapshot_handle();
        let actor = RoutingActor::spawn(RoutingActor::new(snapshot.clone()));
        let tool = RouteDelegationToPeerTool::new(actor);
        ...
        assert_eq!(policy.session_target, RouteTarget::Peer("gpu-box".into()));
    }

    #[tokio::test]
    async fn test_route_delegation_to_peer_resets_to_local() {
        let snapshot = new_routing_snapshot_handle();
        let actor = RoutingActor::spawn(RoutingActor::new(snapshot.clone()));
        let tool = RouteDelegationToPeerTool::new(actor);
        let ctx = AgentToolContext::basic("test-session".to_string(), None);
        ...
        // Then reset to local (peer_name = null)
        let result = tool
            .call(json!({ "agent_id": "coder", "peer_name": null }), &ctx)
            .await
            .expect("call should succeed");

        assert!(result.contains("locally"), "should mention local routing");
    ...
    #[tokio::test]
    async fn test_route_delegation_missing_agent_id() {
        let snapshot = new_routing_snapshot_handle();
