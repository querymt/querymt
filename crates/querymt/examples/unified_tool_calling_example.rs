...
async fn run_simple_scenario(llm: &Box<dyn LLMProvider>) -> Result<(), Box<dyn Error>> {
    ...
    let messages = vec![ChatMessage::user()
        .content("What's the current weather in Tokyo? you should use the provided tools. i'm actually providing you a tool for checking it.")
        .build()];
